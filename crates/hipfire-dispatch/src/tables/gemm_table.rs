...
pub fn populate(registry: &mut KernelRegistry) {
    ...
    registry.register(KernelVariant {
        // Batched IQ3_S GEMM (GSQ-RCO native): same scalar pattern as
        // GemmQ4KBatched (110 B/group, 512-entry grid codebook).
        // Unrotated — Plain only, Always arch gate.
        key: KernelKey::GemmIQ3SBatched,
        arch_required: ArchPredicate::Always,
        shape_gate: None,
        steps: &[PipelineOp::Gemv],
        has_awq: false,
    ...
    registry.register(KernelVariant {
        // Batched IQ2_S GEMM (GSQ-RCO native): same scalar pattern as
        // GemmQ4KBatched (82 B/group, 1024-entry u64 grid). Unrotated —
        // Plain only, Always arch gate.
        key: KernelKey::GemmIQ2SBatched,
        arch_required: ArchPredicate::Always,
        shape_gate: None,
        steps: &[PipelineOp::Gemv],
        has_awq: false,
