...
impl GemmFamily {
    ...
    pub fn run_key(
        ...
        match key {
            ...
            K::GemmIQ3SBatched => {
                hip!(gpu.gemm_iq3_s_batched(w.buf, x, y, m, k, batch_size))
            }
            K::GemmIQ3XXSBatched => {
                hip!(gpu.gemm_iq3_xxs_batched(w.buf, x, y, m, k, batch_size))
            }
            K::GemmIQ2SBatched => {
                hip!(gpu.gemm_iq2_s_batched(w.buf, x, y, m, k, batch_size))
            }
            K::GemmIQ2XSBatched => {
                hip!(gpu.gemm_iq2_xs_batched(w.buf, x, y, m, k, batch_size))
            }
            K::GemmIQ2XXSBatched => {
                hip!(gpu.gemm_iq2_xxs_batched(w.buf, x, y, m, k, batch_size))
            }
