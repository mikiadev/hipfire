...
pub(crate) fn forward_batch_chunk_impl(
    ...
    for layer_idx in layer_start..layer_end {
        match (&weights.layers[layer_idx], config.layer_types[layer_idx]) {
            ...
            (LayerWeights::FullAttnEscha(_layer), LayerType::FullAttention) => {
                // FA fallback: per-token gather/scatter through forward_scratch_layers.
                // FA layers need batched KV cache write + attention, which is not
                // yet wired for Escha. Correct (proven path) but not fast.
                for i in 0..n {
                    let pos = start_pos + i;
                    gpu.hip.memcpy_dtod_at(
                        &s.x.buf,
