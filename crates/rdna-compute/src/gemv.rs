...
impl Gpu {
    ...
    /// y = A_iq3s * x (quantized matrix-vector multiply, A stored as IQ3_S on GPU)
    /// a_raw: raw IQ3_S bytes on GPU, x: F32 input, y: F32 output
    /// m: number of output rows, k: number of input columns (must be multiple of 256)
    pub fn gemv_iq3_s(
        &mut self,
        a_raw: &GpuTensor,
        ...
        self.bind_thread()?;
        self.ensure_kernel("gemv_iq3_s", kernels::GEMV_IQ3S_SRC, "gemv_iq3_s")?;
        let func = &self.functions["gemv_iq3_s"];
    ...
    pub fn gemv_iq3_xxs_q8dot(
        ...
        self.ensure_kernel(
            "gemv_iq3_xxs_q8dot",
            kernels::GEMV_IQ3XXS_Q8DOT_SRC,
            "gemv_iq3_xxs_q8dot",
        )?;
        let func = &self.functions["gemv_iq3_xxs_q8dot"];
        let mut a_ptr = a_raw.buf.as_ptr();
        let mut xs_ptr = xs.buf.as_ptr();
        let mut sc_ptr = xscales.buf.as_ptr();
        let mut y_ptr = y.buf.as_ptr();
        ...
        let bytes = crate::profile::gemv_iq_bytes(m, k, 98);
        let timer = crate::profile::begin_timer(&self.hip, "gemv", "gemv_iq3_xxs_q8dot", bytes);
        let grid = ((m as u32) + 1) / 2;
        let result = unsafe {
            self.hip.launch_kernel(
    ...
    /// y += A_iq3xxs @ q8_1(x): fused-residual IQ3_XXS q8dot.
    pub fn gemv_iq3_xxs_q8dot_residual(
        &mut self,
        a_raw: &GpuTensor,
        xs: &GpuTensor,
        xscales: &GpuTensor,
        ...
        self.ensure_kernel(
            "gemv_iq3_xxs_q8dot",
            kernels::GEMV_IQ3XXS_Q8DOT_SRC,
            "gemv_iq3_xxs_q8dot_residual",
        )?;
        let func = &self.functions["gemv_iq3_xxs_q8dot_residual"];
        let mut a_ptr = a_raw.buf.as_ptr();
        let mut xs_ptr = xs.buf.as_ptr();
        let mut sc_ptr = xscales.buf.as_ptr();
        let mut y_ptr = y.buf.as_ptr();
        ...
        let bytes = crate::profile::gemv_iq_bytes(m, k, 98);
        let timer =
            crate::profile::begin_timer(&self.hip, "gemv", "gemv_iq3_xxs_q8dot_residual", bytes);
        let grid = ((m as u32) + 1) / 2;
        let result = unsafe {
            self.hip.launch_kernel(
                func, [grid, 1, 1], [32, 1, 1], 0, self.stream_ref(), &mut params,
    ...
    /// y = A_iq4xs @ q8_1(x): IQ4_XS twin of gemv_iq3_s_q8dot.
    pub fn gemv_iq4_xs_q8dot(
        &mut self,
        a_raw: &GpuTensor,
        xs: &GpuTensor,
        xscales: &GpuTensor,
        ...
        self.ensure_kernel(
            "gemv_iq4_xs_q8dot",
            kernels::GEMV_IQ4XS_Q8DOT_SRC,
            "gemv_iq4_xs_q8dot",
        )?;
        let func = &self.functions["gemv_iq4_xs_q8dot"];
        let mut a_ptr = a_raw.buf.as_ptr();
        let mut xs_ptr = xs.buf.as_ptr();
        let mut sc_ptr = xscales.buf.as_ptr();
        let mut y_ptr = y.buf.as_ptr();
        ...
        let bytes = crate::profile::gemv_iq_bytes(m, k, 136);
        let timer = crate::profile::begin_timer(&self.hip, "gemv", "gemv_iq4_xs_q8dot", bytes);
        let grid = ((m as u32) + 1) / 2;
        let result = unsafe {
            self.hip.launch_kernel(
                func, [grid, 1, 1], [32, 1, 1], 0, self.stream_ref(), &mut params,
    ...
    /// y += A_iq4xs @ q8_1(x): fused-residual IQ4_XS q8dot.
    pub fn gemv_iq4_xs_q8dot_residual(
        &mut self,
        a_raw: &GpuTensor,
        xs: &GpuTensor,
        xscales: &GpuTensor,
        ...
        self.ensure_kernel(
            "gemv_iq4_xs_q8dot",
            kernels::GEMV_IQ4XS_Q8DOT_SRC,
            "gemv_iq4_xs_q8dot_residual",
        )?;
        let func = &self.functions["gemv_iq4_xs_q8dot_residual"];
        let mut a_ptr = a_raw.buf.as_ptr();
        let mut xs_ptr = xs.buf.as_ptr();
        let mut sc_ptr = xscales.buf.as_ptr();
        let mut y_ptr = y.buf.as_ptr();
        ...
        let bytes = crate::profile::gemv_iq_bytes(m, k, 136);
        let timer =
            crate::profile::begin_timer(&self.hip, "gemv", "gemv_iq4_xs_q8dot_residual", bytes);
        let grid = ((m as u32) + 1) / 2;
        let result = unsafe {
            self.hip.launch_kernel(
                func, [grid, 1, 1], [32, 1, 1], 0, self.stream_ref(), &mut params,
    ...
    /// y = A_iq2s * x (quantized matrix-vector multiply, A stored as IQ2_S on GPU)
    pub fn gemv_iq2_s(
        &mut self,
        a_raw: &GpuTensor,
        x: &GpuTensor,
        ...
        self.bind_thread()?;
        self.ensure_kernel("gemv_iq2_s", kernels::GEMV_IQ2S_SRC, "gemv_iq2_s")?;
        let func = &self.functions["gemv_iq2_s"];
        let mut a_ptr = a_raw.buf.as_ptr();
        let mut x_ptr = x.buf.as_ptr();
        let mut y_ptr = y.buf.as_ptr();
        let mut m_val = m as i32;
    ...
    /// y = A_iq2_xs * x (quantized matrix-vector multiply, A stored as IQ2_XS on GPU)
    pub fn gemv_iq2_xs(
        &mut self,
        a_raw: &GpuTensor,
        x: &GpuTensor,
        ...
        self.bind_thread()?;
        self.ensure_kernel("gemv_iq2_xs", kernels::GEMV_IQ2XS_SRC, "gemv_iq2_xs")?;
        let func = &self.functions["gemv_iq2_xs"];
        let mut a_ptr = a_raw.buf.as_ptr();
        let mut x_ptr = x.buf.as_ptr();
        let mut y_ptr = y.buf.as_ptr();
        let mut m_val = m as i32;
    ...
    /// y = A_iq2_xxs * x (quantized matrix-vector multiply, A stored as IQ2_XXS on GPU)
    pub fn gemv_iq2_xxs(
        &mut self,
        a_raw: &GpuTensor,
        x: &GpuTensor,
        ...
        self.bind_thread()?;
        self.ensure_kernel("gemv_iq2_xxs", kernels::GEMV_IQ2XXS_SRC, "gemv_iq2_xxs")?;
        let func = &self.functions["gemv_iq2_xxs"];
        let mut a_ptr = a_raw.buf.as_ptr();
        let mut x_ptr = x.buf.as_ptr();
        let mut y_ptr = y.buf.as_ptr();
        let mut m_val = m as i32;
