...
pub(crate) enum QuantType {
    ...
    IQ3S = 46,
    /// IQ3_XXS (qt=52): GGUF-native IQ3_XXS passthrough (GSQ-RCO). 98 B per
    /// 256-element group: `[0..2)` fp16 d, `[2..66)` qs[64] grid indices,
    /// `[66..98)` scales_and_signs[32] (8 x u32 aux32). Unrotated (Plain),
    /// K%256==0. Served by `gemv_iq3_xxs` / `gemm_iq3_xxs_batched`.
    IQ3XXS = 52,
    /// IQ2_S (qt=53): GGUF-native IQ2_S passthrough (GSQ-RCO). 82 B per
    /// 256-element group: `[0..2)` fp16 d, `[2..66)` qs[64] (grid idx +
    /// signs), `[66..74)` qh[8], `[74..82)` scales[8]. Unrotated (Plain),
    /// K%256==0. Served by `gemv_iq2_s` / `gemm_iq2_s_batched`.
    IQ2S = 53,
    /// IQ2_XS (qt=54): GGUF-native IQ2_XS passthrough (GSQ-RCO). 74 B per
    /// 256-element group. Unrotated (Plain), K%256==0.
    IQ2XS = 54,
    /// IQ2_XXS (qt=55): GGUF-native IQ2_XXS passthrough (GSQ-RCO). 66 B per
    /// 256-element group. Unrotated (Plain), K%256==0.
    IQ2XXS = 55,
}
