...
7. **Batched-prefill gate**: ⚠️ CODE LANDED, **DEFAULT OFF**. `qwen35_layer_batch_admissible` had returned `Err` for `DeltaNetEscha`/`FullAttnEscha` long after `9daf925bf` "fixed" the attractor, so `prefill_batch_pbs_eligible()` was false for the whole model and prefill silently ran the per-token fallback ("byte-identical to decode") — that is why pp == tg. Re-opening measures **3.5× on prefill** (TTFT 16.8s → 4.8s, 58-token prompt; 10.8× at 600 tokens), **but the batched path is not numerically equivalent to per-token**: at `--temp 0` with identical prompt bytes the two arms differ from the first generated token. Per-token is the host-validated arm, so batched is wrong. It is latent in the batched body (reverting the kernel specialization and the slice heuristic reproduces it) and not a chunk-carry bug (2-row chunks diverge like 58-row ones). Opt in with `HIPFIRE_ESCHA_DENSE_BATCHED=1` only if you want fast-and-wrong. Next step is a batched-vs-decode projection oracle — `check_escha_dense` covers only the decode gemv, which is why this survived.
